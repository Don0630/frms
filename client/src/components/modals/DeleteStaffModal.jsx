...
export default function DeleteStaffModal({ staff, onClose, onConfirm, loading = false }) {
  return (
        ...
        {staff?.FirstName && (
          <p className="text-sm mt-2 text-gray-800 dark:text-gray-200">
            <span className="font-medium">
              {staff.FirstName} {staff.MiddleName ? `${staff.MiddleName[0]}.` : ""} {staff.LastName}
            </span>
            {staff.Department && (
              <span className="text-gray-500 dark:text-gray-400"> — {staff.Department}</span>
            )}
          </p>
        )}
