...
export default function DeleteSubsidyModal({ subsidy, onClose, onConfirm, loading = false }) {
  return (
    <Modal onClose={onClose} width="max-w-sm">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
          Are you sure you want to remove this subsidy?
        </p>

        {subsidy?.ProgramName && (
          <p className="text-sm mt-2 text-gray-800 dark:text-gray-200">
            <span className="font-medium">{subsidy.ProgramName}</span>
            {subsidy.DistributionDate && (
              <span className="text-gray-500 dark:text-gray-400"> — {subsidy.DistributionDate}</span>
            )}
          </p>
        )}
