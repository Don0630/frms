...
export default function DeleteLivestockModal({ livestock, onClose, onConfirm, loading = false }) {
  return (
    <Modal onClose={onClose} width="max-w-sm">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
          Are you sure you want to remove this livestock?
        </p>

        {livestock?.Type && (
          <p className="text-sm mt-2 text-gray-800 dark:text-gray-200">
            <span className="font-medium">{livestock.Type}</span>
            {livestock.Breed && (
              <span className="text-gray-500 dark:text-gray-400"> — {livestock.Breed}</span>
            )}
          </p>
        )}
