...
export default function DeleteMonitoringModal({ monitoring, onClose, onConfirm, loading = false }) {
  return (
        ...
        {monitoring?.FirstName && (
          <p className="text-sm mt-2 text-gray-800 dark:text-gray-200">
            <span className="font-medium">
              {monitoring.FirstName} {monitoring.MiddleName ? `${monitoring.MiddleName[0]}.` : ""} {monitoring.LastName}
            </span>
            {monitoring.ReportDate && (
              <span className="text-gray-500 dark:text-gray-400"> — {monitoring.ReportDate}</span>
            )}
          </p>
        )}
