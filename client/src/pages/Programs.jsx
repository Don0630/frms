...
console.log("programsQuery", programsQuery);
  ...
  // ================= PAGINATION =================
  const { currentPage, setCurrentPage, currentItems, totalPages } = usePagination(filteredData, 10);
  ...
  useEffect(() => {
    if (programsQuery.isError) {
      const code = programsQuery.error?.response?.data?.code;
      const message = programsQuery.error?.response?.data?.message;
  ...
  const columns = [
    ...
    { key: "StartDate", label: "Start", render: (item) => formatDateNumeric(item.StartDate) },
    { key: "EndDate", label: "End", render: (item) => formatDateNumeric(item.EndDate) },
    { key: "Budget", label: "Budget", render: (item) => `₱ ${Number(item.Budget || 0).toLocaleString()}` },
    {
      key: "Status",
      label: "Status",
      render: (item) => (
        <span className={`px-2 py-1 rounded text-white text-xs ${
          ...
          item.Status === "Dropped" ? "bg-red-600" : "bg-gray-400"
        }`}>
          {item.Status}
        </span>
      ),
  ...
  // ================= LOADING =================
  if (programsQuery.isLoading) return <TablePageSkeleton />;
    ...
    <div className="w-full px-4">
      <div className="w-full rounded-sm bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-md p-6 space-y-4">
        ...
        <div className="flex flex-wrap justify-between items-center gap-3 mb-4">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100">ALL PROGRAMS</h2>
          <button onClick={() => setAddProgramModal(true)} className={pageButtonPrimary}>
